...
Ortalama değer aşağıdaki formül ile hesaplanır.
...
- Örneklem uzayı için payda $n-1$, popülasyon için payda $n$ olmalıdır.
...
Burada `pandas` ile `numpy` modüllerinin standart sapma (varyans) değerleri aynı değil. Bunun sebebi istatistikte "yanlı/yansız (biased/unbiased)" olarak bilinen iki farklı yaklaşım olmasıdır. Bu iki yaklaşımın farklılıkları için aşağıdaki linkleri inceleyebilirsiniz.
