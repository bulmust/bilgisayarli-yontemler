...
Bu şekilde bir dalganın frekansı nedir sorusunu cevaplayamayız. Benzer şekilde üst üste binen dalgaların genlik bilgileri de açıkça gözükmemektedir.

Bu ve bunun gibi dalgaları analiz etmek için **Fourier dönüşümü** kullanılır. Fourier dönüşümü, bir dalgayı frekans bileşenlerine ayırır. Fourier dönüşümünü kesikli (bizim inceleyeceğimiz durum) ve sürekli olmak üzere iki farklı şekilde yapabiliriz. 
...
Formül bize kabaca şunu söylemektedir. Eğer $N$ adet örnek sayımız varsa, bunu $k=0,\cdot,N-1$ adet frekansa sahip dalgaların toplamı şeklinde düşünebiliriz. $k$ frekansına sahip olan dalganın değeri $X_{k}$, faz değeri ise $\phi_{k}$ olacaktır.
...
Yukarıdaki gibi örnek oranının yarısına **Nyquist frekansı** denir. Yani $N-1$ üzerinden toplam almak yerine $N/2$ üzerinden toplam alırsak Nyquist frekansına ulaşırız. İlerleyen konularda dalgaların frekanslarını bulurken bu konu daha ayrıntılı açıklanacaktır.
...
```{python}
import numpy as np
# Kesikli Fourier Dönüşümü Fonksiyonu
...
%timeit fourier_kfd(olus_sin(orneklemOrani1))
print("\n")
# Orneklem Orani 2
orneklemOrani2 = 200
# Toplam zamanı hesapla
...
print(" Fouirer dönüşümü yapması için geçen süre")
# FD Fonksiyonunu çağır ve %temit ile süreyi ölç
%timeit fourier_kfd(olus_sin(orneklemOrani2))
```
...
1. Zaman Uzayı
  - **Örneklem sayısı, blok boyutu (# of samples, block size)**: Bir sinyali kaç adet örnek ile betimliyoruz [adet], `N`.
  - **Örneklem oranı, örnekleme frekansı (sampling rate, sampling frequency)**: Bir saniye içerisinde kaç örneklem olduğunu veren ifade [1/saniye] `orneklemOrani`.
  - **Örnekleme aralığı (sampling interval)**: İki örneklem arasının kaç saniye olduğunu veren ifade [saniye], `ts`.
  - **Zaman, pencere boyutu (time, frame size)**: Bir sinyalin kaç saniye içerisinde olduğunu veren ifade [saniye], `t`.
1. Frekans Uzayı
  - **Maksimum Frekans, Nyquist Frekansı, bant aralığı (maximum frequency, Nyquist frequency, bandwith)**: Bir sinyalin içerisindeki en yüksek frekans [Hz], `maxFrek`. Maksimum frekans değeri, örneklem oranının yarısına eşittir, `maxFrek=orneklemOrani/2`.
  ...
  - **Frekans Çözünürlülüğü**: Frekans uzayindaki iki frekans arasındaki fark [Hz], `frekCoz`.
...
::: {.callout-warning}
`t=np.arange(-2, 2, orneklemAraligi)` kodunda $N=100$ adet nokta vardır ancak $2$ noktası alınmamıştır.
:::

- Maksimum frekans değeri $25/2=12.5$ [Hz] olur, `maksFrek=N/2`.
- Frekans çözünürlülüğü $0.5$ [Hz] olur, `frekCoz=maksFrek/N`.
- Tüm frekanslar `frek= np.arange(-maksFrek, maksFrek, frekCoz)` olur.
...
Açısal frekansı 5, genliği 10 olan bir cosinüs sinyali çizdirin. Sinyalin KFD'sini alın ve genlik ve faz değerlerini bulun.
